...
object WordCount {
  def main(args: Array[String]): Unit = {
    ...
    val wordStream = initStream.flatMap(_.split(" ")).setParallelism(1)
    val pairStream = wordStream.map((_,1)).setParallelism(1)
    val keyByStream = pairStream.keyBy(0)
    val restStream = keyByStream.sum(1).setParallelism(1)
    restStream.print()
